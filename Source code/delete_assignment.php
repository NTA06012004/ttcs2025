...
try {
    ...
    $stmt_get_submission_files = $conn->prepare("SELECT submission_file FROM submissions WHERE assignment_id = ? AND submission_file IS NOT NULL");
    if ($stmt_get_submission_files) {
        ...
        while($file_row = $submission_files_result->fetch_assoc()){
            $submission_file_to_delete = 'uploads/submission_files/' . $file_row['submission_file'];
