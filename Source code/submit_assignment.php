...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && !$is_view_mode) {
    if (!$submission_allowed) {
        ...
        $file_path_for_db = $existing_submission['submission_file'] ?? null; // Giữ file cũ nếu không tải lên file mới khi nộp lại
        ...
        if (isset($_FILES['submission_file']) && $_FILES['submission_file']['error'] == UPLOAD_ERR_OK) {
            ...
            if(!isset($errors['file'])){
                ...
                if (empty($errors['file'])) {
                    ...
                    if ($existing_submission && $existing_submission['submission_file'] && $existing_submission['submission_file'] != $new_file_name) {
                        $old_file_to_delete = $upload_dir . $existing_submission['submission_file'];
        ...
        if (empty($errors)) {
            ...
            if ($existing_submission) { // Cập nhật bài nộp cũ (nếu $can_resubmit = true)
                $stmt_update = $conn->prepare("UPDATE submissions SET submission_text = ?, submission_file = ?, status = ?, submitted_at = ? WHERE id = ?");
                if ($stmt_update) {
                ...
                $stmt_insert = $conn->prepare("INSERT INTO submissions (assignment_id, student_id, submission_text, submission_file, status, submitted_at) VALUES (?, ?, ?, ?, ?, ?)");
                if ($stmt_insert) {
            ...
            <div class="card-body p-4">
                ...
                <?php if ($is_view_mode): ?>
                    ...
                    <?php if (!empty($existing_submission['submission_file'])): ?>
                        ...
                        <p><a href="uploads/submission_files/<?php echo htmlspecialchars($existing_submission['submission_file']); ?>" target="_blank" download class="btn btn-outline-success btn-sm"><i class="bi bi-download me-1"></i><?php echo htmlspecialchars($existing_submission['submission_file']); ?></a></p>
                    <?php endif; ?>
                    <?php if (empty($existing_submission['submission_text']) && empty($existing_submission['submission_file'])): ?>
                    ...
                    <form action="submit_assignment.php?assignment_id=<?php echo $assignment_id; ?>" method="POST" enctype="multipart/form-data" novalidate>
                        ...
                        <div class="mb-3">
                            ...
                            <?php if($existing_submission && $existing_submission['submission_file']): ?>
                                <p class="form-text">Tệp hiện tại: <a href="uploads/submission_files/<?php echo htmlspecialchars($existing_submission['submission_file']); ?>" target="_blank"><?php echo htmlspecialchars($existing_submission['submission_file']); ?></a>. Tải tệp mới sẽ thay thế tệp này.</p>
                            <?php endif; ?>
